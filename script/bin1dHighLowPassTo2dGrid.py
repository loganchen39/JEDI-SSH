...
DIR_DATA = '/glade/scratch/lgchen/data/OISSH_NOAA/2000-2022_fromEric/FourierFilter'

ds_ssh_ana = xr.open_dataset(filename_or_obj=DIR_DATA+'/misc/ana.2022011512.withCoords.nc')
lat_oi = ds_ssh_ana.coords['lat']  # 720: -89.875, -89.625, -89.375, ...,  89.375,  89.625,  89.875
lon_oi = ds_ssh_ana.coords['lon']  # 1440: -179.875, -179.625, ..., 179.875  # 0.125 0.375 0.625 ... 359.625 359.875
...
daily_sla_avg_iq2oi = np.zeros((1440, 720, 3, 31), dtype=np.float32, order='F')  # 3 for daytime, nighttime and allday
daily_sla_num_iq2oi = np.zeros((1440, 720, 3, 31), dtype=np.int32  , order='F')  # for binned to avg
